 ...
 * 
 * 
 * 編碼速度可以儲存在16 bit 的數字，這樣就可以以一個short大小的變量，儲存三種不同單位的數
 * 值。速度單位的種類以二進制頭2至4個位作判定。
 *
...
class speed_phase {
 public:
  ...
  speed_phase(short speed)
      : type(TIME_PHASE), time(0), speed(speed) {}
  explicit speed_phase(speed_control_phase_type_e_t type, long ms, double val, short speed)
      : type(type), time(ms), value(val), speed(speed) {}
